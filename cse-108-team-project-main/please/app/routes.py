...
# blueprint created to help sort the routes
bp = Blueprint('routes', __name__)

@bp.route('/')
def index():
    if current_user.is_authenticated:
        #direct based on role, if student then student dashboard, if teacher then teacher dashboard, if admin then admin dashboard
...
def student_dashboard():
    ...
    # Shows courese the student is NOT enrolled in
    enrolled_course_ids = [e.course_id for e in my_enrollments]
    
    # Query for courses not enrolled in
...
def enroll_course(course_id):
    ...
    #Checks that the student needs to pass to register for a class
    ...
    #If passed all the checks, student is enrolled
...
#Teacher dashboard and UI

@bp.route('/teacher')
@login_required
def teacher_dashboard():
    if current_user.role_id != 2: #checks if you are a teacher/ have a teacher role
        flash("Access denied: You must be a Teacher.")
        return redirect(url_for('routes.index'))

    #shows all the coruses taught by a teacher
...
def view_class_roster(course_id):
    ...
    # check to make sure the teacher is teaching a class they are trying to view
    if not course or course.teacher_id != current_user.id:
        flash("Course not found or you are not the assigned teacher.")
        return redirect(url_for('routes.teacher_dashboard'))

    # shows the enrolled students in the class and their grades
...
def update_grade(enrollment_id):
    ...
    # Checks to see if the teacher is updating a grade for their own course
    if not enrollment or enrollment.course.teacher_id != current_user.id:
        ...
        return redirect(url_for('routes.teacher_dashboard'))
    #checks to make sure the new grade is valid 
    try:
    ...
    #redurects back to class roster after updating grade
    return redirect(url_for('routes.view_class_roster', course_id=enrollment.course_id))

#HTML formating

# Helpter function that mimics Flash messages in render_template
from flask import render_template_string, get_flashed_messages

# Student Dashboard HTML formate
